...
export function layoutCanvasGraph(nodes: CanvasNodeData[], connections: CanvasConnection[], targetIds: string[]) {
  ...
  const incomingCount = new Map(targets.map((node) => [node.id, 0]));
  ...
  const visit = (nodeId: string, stack = new Set<string>()): number => {
    ...
    stack.add(nodeId);
    const parents = connections.filter((connection) => connection.toNodeId === nodeId && targetSet.has(connection.fromNodeId));
    const value = parents.length ? Math.max(...parents.map((connection) => visit(connection.fromNodeId, stack) + 1)) : 0;
    stack.delete(nodeId);
  ...
  sortedColumns.forEach(([, columnNodes]) => {
    columnNodes.sort((a, b) => (incomingCount.get(a.id) ?? 0) - (incomingCount.get(b.id) ?? 0) || a.position.y - b.position.y);
    let y = originalMinY;
    const columnWidth = Math.max(...columnNodes.map((node) => node.width));
